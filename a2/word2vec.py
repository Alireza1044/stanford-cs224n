...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs = np.dot(y_hat.reshape((-1,1)), centerWordVec.reshape((-1,1)).T)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    uo = np.expand_dims(outsideVectors[outsideWordIdx],axis=1)
    centerWordVec = np.expand_dims(centerWordVec, axis=1)

    d = np.log(sigmoid(uo.T.dot(centerWordVec)))
    s = np.sum(np.log(sigmoid(-uk.dot(centerWordVec))),axis=0)
    loss = -np.squeeze(d) - s

    s = np.sum(uk*(1 - sigmoid(-uk.dot(centerWordVec))),axis=0)
    d = uo * (1 - sigmoid(uo.T.dot(centerWordVec)))
    gradCenterVec = s - np.squeeze(d)

    uo = np.squeeze(uo)
    centerWordVec = np.squeeze(centerWordVec)
    
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = np.squeeze((sigmoid(uo.dot(centerWordVec)) - 1) * centerWordVec)
    unique,inverse, counts = np.unique(negSampleWordIndices, return_counts=True,return_inverse=True)

    uk_deriv = sigmoid(-uk.dot(centerWordVec))

    for i, idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[idx] = centerWordVec * (1 - uk_deriv[i])
    for i in range(len(unique)):
      gradOutsideVecs[unique[i]] *= counts[i]
    ### END YOUR CODE

    return loss, gradCenterVec, gradOutsideVecs # (10,3)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for i in range(len(outsideWords)):
      l, gcv, gov = word2vecLossAndGradient(centerWordVector, outsideWordsIdx[i], outsideVectors, dataset)
      loss += l
